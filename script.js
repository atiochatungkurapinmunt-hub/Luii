...
function setupMusicPlayer() {
    ...
    if (config.music.type === 'spotify' && config.music.spotifyUrl) {
        // Hide the MP3 controls
        musicControls.style.display = 'none';

        // Extract Spotify track ID from URL
        const match = config.music.spotifyUrl.match(/track\/([a-zA-Z0-9]+)/);
        if (match && match[1]) {
